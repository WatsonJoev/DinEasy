...
export function OrderCustomizationModal({ open, onOpenChange, menuItem, onAddToCart }: OrderCustomizationModalProps) {
  ...
  // Initialize all states, but reset when menuItem or open changes
              ...
              <div className="text-right">
                ...
                <Badge
                  ...
                  className={
                    ...
                    (menuItem.foodType === 'others' && ['Desserts', 'Beverages', 'Snacks'].includes(menuItem.category))
                      ? 'bg-olivine text-white' 
                      : 'bg-pumpkin text-white'
                  }
                >
                  {menuItem.foodType === 'veg' ? '🌱 Veg' : 
                   menuItem.foodType === 'non-veg' ? '🍗 Non-Veg' :
                   '🍦 Others'}
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {['mild', 'medium', 'hot'].map((level) => (
                <Button
                  ...
                  onClick={() => setSpiceLevel(level as 'mild' | 'medium' | 'hot')}
